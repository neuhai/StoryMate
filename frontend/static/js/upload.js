...
function step1(){
    document.querySelector('.uploadStep').innerHTML = `<p1>第一步</p1>
    </br>
    <p2>上传您书本的封面</p2>`;
...
function shootCover(){
    ...
    <button class="capture-btn" onclick="captureCoverImg()">点击拍摄</button>`;
...
function captureCoverImg(){
    ...
    <button id="retake-btn" onclick="shootCover()">重新拍摄</button> 
    <div class='cameraBtns'>                                                         
        <input id="uploadTitle" placeholder="给这本书起一个标题吧！"/>                                   
        <button class="confirm-btn" onclick="confirmCover()">确认</button>
    </div>`;
...
function step2(){
    document.querySelector('.uploadStep').innerHTML = `<p1>第二步</p1>
    </br>
    <p2>请为</p2><p3>${bookTitle}</p3><p2>的每一页进行上传</p2>`;
    document.getElementById('workPlace').innerHTML = `<div class="uploadContent">
...
function shootPage(){
    ...
    <button class="capture-btn" onclick="capturePageImg()">点击拍摄</button>`;
    showCamera('page');
}
...
async function capturePageImg(){
    ...
    (async () => {
        ...
        if (curPage > 1){
            document.querySelector('.uploadContent').innerHTML = `<img id="captured-image" src=${canvas.toDataURL('image/png')} alt="Captured Image"></img>
            <button id="retake-btn" onclick="shootPage()">重新拍摄</button> 
            <div class='modifyUpload'>                                                         
                <textarea id="ocrText">${curText}</textarea>                                  
            </div>
            <button id="prev-btn" onclick="savePrev('${canvas.toDataURL('image/png')}')">上一页</button>
            <button id="save-btn" onclick="saveContinue('${canvas.toDataURL('image/png')}')">保存并继续</button>
            <button id="end-upload-btn" onclick="finishUpload('${canvas.toDataURL('image/png')}')">结束上传！</button>`;  
        }
        else{
            document.querySelector('.uploadContent').innerHTML = `<img id="captured-image" src=${canvas.toDataURL('image/png')} alt="Captured Image"></img>
            <button id="retake-btn" onclick="shootPage()">重新拍摄</button> 
...
function savePrev(imgSrc){
    ...
    document.querySelector('.uploadContent').innerHTML = `<img id="captured-image" src=${pageImg[curPage].img} alt="Captured Image"></img>
        ...
        <button id="prev-btn" onclick="savePrev('${pageImg[curPage].img}')">上一页</button>
        <button id="save-btn" onclick="saveContinue('${pageImg[curPage].img}')">保存并继续</button>
        <button id="end-upload-btn" onclick="finishUpload('${pageImg[curPage].img}')">结束上传！</button>`;   
}
...
function step3(disciplines){
    ...
    document.querySelector('.uploadStep').innerHTML = `<p1>科学学科匹配完成！</p1>
    </br>
    <p2>与</p2><p3>${bookTitle}</p3><p2>最相关的科学学科是</p2><p4>${disc[0][0]}</p4>
    <button id="backtoCust" onclick="toCust()">明白了！</button>`;
}
