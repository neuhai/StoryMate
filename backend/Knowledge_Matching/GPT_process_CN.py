...
from Text_Process.Text_Process import split_para
from .Keyword_Matching_CN import keyword_matching
...
def prompt_greeting():
    Instruction = f"""\
现在你是一个与6-8岁儿童互动的对话的友善的伙伴，名叫智星，你现在需要给小朋友介绍自己，和接下来的互动讲故事活动，并通过互动来得到小朋友的背景信息。
...
问题3: [对小朋友的回答做出肯定、积极的回复]你有什么喜欢的话题吗？比如太空、公主、恐龙或者是汽车呢，你可以说说任何你喜欢的东西！
...
{{
    ...
    "language_literacy": [小朋友的语言水平，格式为字符串，如果没有，就返回空字符串""],
...
def prompt_conv(title, story, keyword, knowledge, statements, info):
    Instruction4 = f"""\
现在你是一个与6-8岁儿童一起阅读故事书的对话的友善的伙伴，名叫智星，你需要通过提问和提供回应来与孩子互动，教他/她基于一段故事的科学知识。
以下是孩子的个人信息：
{info}

请遵循以下要求与孩子进行互动：
1. 你和孩子将轮流对话，每轮生成一个问题，总问题数不超过三个。如果孩子对知识有很好理解，应减少问题数量。
2. 你的问题和回复需匹配孩子的年龄，风格亲切、生活化。使用简单、非正式的汉语，就像和同龄朋友聊天一样。不要使用敬语。对5-6岁的孩子，语言应简单易懂、生动有趣，避免使用复杂词汇和冗长解释。提问时自然些，让孩子觉得是在探索科学世界，而不是完成任务。
3. 在故事文本中会有一个概念词，这个词与外部科学知识相关。问题应基于这个概念词，并融入相关科学知识，丰富孩子的科学知识。问题系列应遵循学龄前儿童的科学表现描述，并基于我提供的故事文本、概念词、知识和科学表现描述。
4. 问题和回复应包含孩子感兴趣的话题，但一定要合理而不牵强。如果孩子的个人信息中提到喜欢老虎，可以在问题或回复中提到老虎，例如'假如一只老虎来到海滩，你觉得他会看到什么动物或植物呢？'吸引孩子兴趣。
5. 当孩子回应时，请做出回复。回复应包括：1. 判断回答的正确性，2. 友好、鼓励的反馈，3. 对前一个问题答案的解释，4. 如果对话未结束，转向下一个问题。5.每轮对话只能问一个问题。
6. 如果孩子回答不正确，用引导方法启发他们思考。如果孩子没有回应，可简单回复"没关系，我们一起想想"并解释，或"我没听清你的回答，你可以再说一遍吗？"并重复问题。
7. 保持回应简洁，每部分不超过15个字，尽量使用简单词汇，避免使用专业的科学词汇，比如'液态'，'可逆'等，如果你必须提到专业科学词汇，请务必用通俗、有趣的语言解释这个词汇，保证孩子可以听懂你在说什么。
8. 在聊天过程中，必须避免出现任何不适合儿童的内容，包括但不限于暴力、粗俗语言、色情或任何违背积极向上价值观的内容。

对于你生成的第一个问题，请按照以下json格式，注意json字典里key和value应使用双引号：
{{
    "greeting": [介绍自己，（我是智星/我的名字是智星）向孩子问好并开始对话，例如'你好呀！'，格式为字符串],
    "question": [第一个问题。只包含一个问题。尽量在问题中包含科学知识的同时，包含孩子感兴趣的东西/话题。比如，如果小朋友提到喜欢老虎，你可以在问题或回复中包含老虎的形象，例如'假如一只老虎来到了海滩，你觉得他会看见什么样的动物或植物呢？' 格式为字符串]
}}
...
{{
    ...
    "explanation": [根据孩子的回答对前一个问题的相关知识进行简单易懂的解释，如果孩子答错，一定要解释正确答案和背后的简单原理，不超过25个字，格式为字符串],
    "transition": [从解释到问题或结束对话的转折，例如'让我们一起来探索一下这个现象/问题：'，'让我们继续读！'等。不要在transition中问问题。格式为字符串],
    "question": [新问题，只包含一个问题。尽量在问题中包含孩子感兴趣的东西/话题，但一定要合理而不牵强。比如，如果小朋友提到喜欢老虎，你可以在问题或回复中包含老虎的形象，例如'假如一只老虎来到了海滩，你觉得他会看见什么样的动物或植物呢？' 如果是最后一个反馈，那么这个字符串应该为空。格式为字符串],
...
def generate_persona(user):
    ...
    persona_dict = load_json('../../frontend/static/files/' + user + '/persona.json')
...
knowledge_dict = load_json(os.path.join(os.path.dirname(__file__), "NGSS_DCI_CN.json"))
knowledge_list = [kg for kg in knowledge_dict.keys()]
statements_dict = load_json(os.path.join(os.path.dirname(__file__), "NGSS_statements_CN.json"))
...
def generate_sum_conv(title, user, isLibrary):
    if os.path.exists('../../frontend/static/files/' + user  + '/' + title +  '/sum_message.json'):
        sum_message = load_json('../../frontend/static/files/' + user  + '/' + title + '/sum_message.json')
        return json.loads(sum_message[1]['content'])
    
    if isLibrary and os.path.exists("../../frontend/static/files/books/" + title + "/sum_kg.json"):
        sum_kg_dict = load_json("../../frontend/static/files/books/" + title + "/sum_kg.json")
        sum = sum_kg_dict['sum']
        kg = sum_kg_dict['kg']
    else:
        if isLibrary:
            book_list = load_json("../../frontend/static/files/books/" + title + "/" + title + '.json')
        else:
            book_list = load_json("../../frontend/static/files/" + user + "/" + title + '/' + title + '.json')
        book_str = ''
    ...
    if not os.path.exists('../../frontend/static/files/' + user  + '/' + title +  '/sum_message.json'):
        save_json('../../frontend/static/files/' + user  + '/' + title +  '/sum_message.json', [])
    sum_message = load_json('../../frontend/static/files/' + user  + '/' + title + '/sum_message.json')
    ...
    if not os.path.exists("../../frontend/static/files/" + user  + '/' + title +  "/sum_audio"):
        os.makedirs("../../frontend/static/files/" + user  + '/' + title +  "/sum_audio")
    audio_gen(first_sum_dict['greeting'] + first_sum_dict['question'], "../../frontend/static/files/" + user  + '/' + title +  "/sum_audio/q_1.mp3")
    save_json('../../frontend/static/files/' + user + '/' + title +  '/sum_message.json', sum_message)
...
def generate_sum_chat(user, title, id, response):
    ...
    audio_gen(feedback_dict['feedback'] + feedback_dict['explanation'] + feedback_dict['transition'] + feedback_dict['question'], "../../frontend/static/files/" + user  + '/' + title + "/sum_audio/q_" + str(id) + ".mp3")
...
def knowledge_keyword_gen(user, title, isLibrary):
    if isLibrary:
        return load_json('../../frontend/static/files/books/' + title + '/' + title + ' Gen.json')

    if os.path.exists('../../frontend/static/files/' + user + '/' + title + '/' + title + ' Gen.json'):
        return load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + ' Gen.json')
    else:
      knowledge_gen = keyword_matching(user, title, isLibrary)
      save_json('../../frontend/static/files/' + user + '/' + title + '/' + title + ' Gen.json', knowledge_gen)  
    return knowledge_gen

def gen_greet(user):
    if os.path.exists('../../frontend/static/files/' + user + '/greet_message.json'):
        greet_message = load_json('../../frontend/static/files/' + user + '/greet_message.json')
    ...
    if not os.path.exists('../../frontend/static/files/' + user + '/greet_message.json'):
        save_json('../../frontend/static/files/' + user + '/greet_message.json', [])
    greet_message = load_json('../../frontend/static/files/' + user + '/greet_message.json')
    first_greet_dict = {
    ...
    if not os.path.exists("../../frontend/static/files/" + user + "/greet_audio"):
        os.makedirs("../../frontend/static/files/" + user + "/greet_audio")
    audio_gen(first_greet_dict['greeting_content'], "../../frontend/static/files/" + user + "/greet_audio/q_1.mp3")
    save_json('../../frontend/static/files/' + user + '/greet_message.json', greet_message)
...
def answer_greet(user, response, id, title, isLibrary):
    ...
    audio_gen(feedback_dict['greeting_content'], "../../frontend/static/files/" + user + "/greet_audio/q_" + str(id) + ".mp3")
...
def update_age_cog(user, title, isLibrary):
    ...
    level = get_level(age)
    if (isLibrary):
        gen_result =  load_json('../../frontend/static/files/books/' + title + '/' + title + ' Gen.json')  
    else:
        gen_result =  load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + ' Gen.json')  
...
def conv_gen(id, title, user, isLibrary):
    ...
    if isLibrary and os.path.exists("../../frontend/static/files/books/" + title + "/conv_audio/") and not os.path.exists("../../frontend/static/files/" + user + "/" + title + "/conv_audio/"):
        os.makedirs("../../frontend/static/files/" + user + "/" + title + "/conv_audio/")
    if not os.path.exists("../../frontend/static/files/" + user + "/" + title + "/conversation/get_conv_sec_" + id + ".json") and os.path.exists("../../frontend/static/files/books/" + title + "/conversation/get_conv_sec_" + id + ".json"):
        save_json("../../frontend/static/files/" + user + "/" + title + "/conversation/get_conv_sec_" + id + ".json", load_json("../../frontend/static/files/books/" + title + "/conversation/get_conv_sec_" + id + ".json"))
        shutil.copy("../../frontend/static/files/books/" + title + "/conv_audio/sec_" + str(id) + "_q_1.mp3", "../../frontend/static/files/" + user + "/" + title + "/conv_audio/sec_" + str(id) + "_q_1.mp3")
    
    if os.path.exists("../../frontend/static/files/" + user + "/" + title + "/conversation/get_conv_sec_" + id + ".json"):
        return json.loads(load_json("../../frontend/static/files/" + user + "/" + title + "/conversation/get_conv_sec_" + id + ".json")[1]['content'])
    if isLibrary:
        sec_dict = load_json('../../frontend/static/files/books/' + title + '/' + title + ' Gen.json')[str(id)]
        section = load_json('../../frontend/static/files/books/' + title + '/' + title + '.json')[int(id)]
    else:
        sec_dict = load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + ' Gen.json')[str(id)]
        section = load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + '.json')[int(id)]
    chatHistory = load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + '_knowledge_dict.json')
    ...
    if not os.path.exists("../../frontend/static/files/" + user + "/" + title + "/conv_audio"):
        os.makedirs("../../frontend/static/files/" + user + "/" + title + "/conv_audio")
    audio_gen(first_conv_dict['greeting'] + first_conv_dict['question'], '../../frontend/static/files/' + user + '/' + title + "/conv_audio/sec_" + str(id) + "_q_1.mp3")

    save_json('../../frontend/static/files/' + user + '/' + title + '/' + title + '_knowledge_dict.json', chatHistory)
    save_json("../../frontend/static/files/" + user + "/" + title + "/conversation/get_conv_sec_" + id + ".json", messages)
    # save_json('../../frontend/static/files/books/' + title + '/' + title + '_knowledge_dict.json', chatHistory)
    # save_json("../../frontend/static/files/books/" + title + "/conversation/get_conv_sec_" + id + ".json", messages)
...
def gen_first_conv(title):
    ...
    for sec_id in range(len(story)):
        conv_gen(str(sec_id), title, 'user', True)

# gen_first_conv("海边度假")

def label_conv_gen(id, title, user, isLibrary):
    if not os.path.exists('../../frontend/static/files/books/' + title + "/conversation/"):
         os.makedirs('../../frontend/static/files/books/' + title + "/conversation/")
    if isLibrary:
        sec_dict = load_json('../../frontend/static/files/books/' + title + '/' + title + ' Gen.json')[str(id)]
        section = load_json('../../frontend/static/files/books/' + title + '/' + title + '.json')[int(id)]
    else:
    ...
    chatHistory = load_json('../../frontend/static/files/books/' + title + '/' + title + '_knowledge_dict.json')
    ...
    if not os.path.exists("../../frontend/static/files/books/" + title + "/conv_audio"):
        os.makedirs("../../frontend/static/files/books/" + title + "/conv_audio")
    audio_gen(first_conv_dict['greeting'] + first_conv_dict['question'], "../../frontend/static/files/books/" + title + "/conv_audio/sec_" + str(id) + "_q_1.mp3")
...
def chat_gen(id, title, user, response):
    messages = load_json("../../frontend/static/files/" + user + "/" + title + "/conversation/get_conv_sec_" + id + ".json")
    chatHistory = load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + '_knowledge_dict.json')
    if response == '':
        response = "[No Response]"
    messages.append(
    ...
    audio_gen(feedback_dict['feedback'] + feedback_dict['explanation'] + feedback_dict['transition'] + feedback_dict['question'], "../../frontend/static/files/" + user + "/" + title + "/conv_audio/sec_" + str(id) + "_q_" + str(len(chatHistory[id]['conversation'])) + ".mp3")
...
def save_knowledge(user, title, isLibrary):
    if (isLibrary):
        gen_data = load_json('../../frontend/static/files/books/' + title + '/' + title + ' Gen.json')
    else:
        gen_data = load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + ' Gen.json')
    gpt_data = {}
    ...
    if (isLibrary):
        save_json('../../frontend/static/files/books/' + title + '/' + title + '_knowledge_dict.json', gpt_data)
    else:
        save_json('../../frontend/static/files/' + user + '/' + title + '/' + title + '_knowledge_dict.json', gpt_data)
...
def get_book_discipline(user, title, isLibrary):
    if (isLibrary):
        gpt_data = load_json('../../frontend/static/files/books/' + title + '/' + title + '_knowledge_dict.json')
    else:
        gpt_data = load_json('../../frontend/static/files/' + user + '/' + title + '/' + title + '_knowledge_dict.json')
    disc = {}
    sub_dis = {}
...
def gen_conv_lib():
    ...
    for b in lib_book:
        kg_dict = load_json('../../frontend/static/files/books/' + b + '/' + b + '_knowledge_dict.json')
